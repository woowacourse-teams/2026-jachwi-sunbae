...
회원 ID를 기준으로 주소·좌표를 포함한 매물, 부가정보·자유 메모와 단계별 체크리스트 스냅샷을 MySQL에 저장한다. 사진 메타데이터는 DB, 바이트는 비공개 객체 저장소에 두고 소유자 검증 백엔드 endpoint로만 조회한다. 새 DB 스키마 정본과 기존 DB의 순방향 보강 절차는 [데이터베이스 초기화](../guides/database-initialization.md)를 따른다.

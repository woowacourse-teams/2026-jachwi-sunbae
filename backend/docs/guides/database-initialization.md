...
| `migration_legacy_stage_counts` | 후속 Contract에서 제거할 레거시 체크리스트 단계의 사전 보존 건수 |
...
- `ONLINE_PHONE`은 다른 단계로 자동 변환하지 않는다. V3은 `migration_legacy_stage_counts`에 기존 시스템 항목 건수를 기록하고, API 전환·검증을 마친 별도 Contract 버전에서 이 스냅샷과 대조한 뒤 제거한다.
- 변환할 수 없는 구조화 메모 값은 NULL로 두고 `migration_backfill_failures`에 원문과 사유를 남긴다. 숫자 형식이지만 유효하지 않은 날짜·시각이나 `BIGINT UNSIGNED` 범위를 벗어난 관리비도 같은 방식으로 기록한다. 실패 기록은 중복 키 갱신으로 보존하며 삽입 오류를 무시하지 않는다. 자유 메모는 기존 `property_memos.free_memo`를 그대로 유지한다.
...
4. Testcontainers MySQL에서 전체 버전 적용, 재기동 시 pending 없음, 기존 데이터 baseline·backfill·관계 데이터 보존과 실패 목록을 검증한다.
